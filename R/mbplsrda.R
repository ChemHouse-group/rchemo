mbplsrda <- function(Xlist, y, scaling = c("none", "pareto", "sd")[1], blockscaling = TRUE, weights = NULL, nlv) {
    if(is.factor(y))
        y <- as.character(y)
    ...
    z <- dummy(y)
    fm <- mbplsr(Xlist, z$Y, scaling = scaling, blockscaling = blockscaling, weights = weights, nlv = nlv)
    structure(
