...
.soplsrdacv <- function(Xlist, y, Xscaling = c("none", "pareto", "sd")[1], Yscaling = c("none", "pareto", "sd")[1], weights = NULL, nlvlist=list(), nbrep = 30, cvmethod = "kfolds", seed = 123, samplingk = NULL, nfolds = 7, optimisation = c("global","sequential")[1], criterion = c("err", "rmse")[1], selection = c("localmin","globalmin","1std")[1]){
  ...
  if((optimisation=="global")|(nXblocks==1)){
    ...
    Rep_errCV <- array(0, dim=c(nrow(lvcombi),nbrep, 1), dimnames = list(paste0("lvcombi",1:nrow(lvcombi)), paste0("rep",1:nbrep), "value"))
    ...
    for(i in 1:nbrep){
      ...
      for(j in 1:nrow(lvcombi)){
        ...
        for(k in 1:length(CVtype[[i]])){
          ...
          repmodel <- soplsrda(Xlist=Xlisttrain, y=Ytrain, Xscaling = Xscaling, Yscaling = Yscaling, weights = weights[-CVtype[[i]][[k]]], nlv=lvcombi[j,])
    ...
    res_errCV[,"mean"] <- apply(Rep_errCV, c(1,3),mean, na.rm=T)
    res_errCV[,"sd"]   <- apply(Rep_errCV, c(1,3),sd, na.rm=T)
  ...
  if((optimisation=="sequential") & (nXblocks>1)){
    ...
    for (m in 1:nXblocks) {
      ...
      Rep_errCV[[m]]  <- array(0, dim=c(nrow(lvcombi[[m]]),nbrep, 1), dimnames = list(paste0("lvcombi",1:nrow(lvcombi[[m]])), paste0("rep",1:nbrep), "value"))
      ...
      for(i in 1:nbrep){
        for(j in 1:nrow(lvcombi[[m]])){
          ...
          for(k in 1:length(CVtype[[i]])){
            ...
            Ytest <- Y[CVtype[[i]][[k]],,drop=FALSE]
            repmodel <- soplsrda(Xlist=Xlisttrain, y=Ytrain, Xscaling = Xscaling, Yscaling = Yscaling, weights = weights[-CVtype[[i]][[k]]], nlv=unlist(lvcombi[[m]][j,]))
            reppredCV[CVtype[[i]][[k]],] <- predict(repmodel, Xlisttest)$posterior[order(CVtype[[i]][[k]]),]
            reppredYCV[CVtype[[i]][[k]],] <- predict(repmodel, Xlisttest)$pred[order(CVtype[[i]][[k]]),]
          }
          SqErrCV <- (dummy(Y)$Y-reppredCV)^2
