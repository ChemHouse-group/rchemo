...
barplot(summary(pcaGCTOF,X = scale(GCTOF[,1:dim(GCTOF)[2]]))$explvar$pvar * 100, names.arg = 1:nrow(GCTOF))
...
plotxy(X= pcaGCTOF$P, group = NULL, 
       ...
       zeroes = TRUE, circle = FALSE, ellipse = FALSE,
       labels = TRUE,
       legend = FALSE, legend.title = NULL, ncol = 1,
       cex=0.8) 
...
## diagram of explained variance
barplot(summary(pcaHILICPOS,X = scale(HILICPOS[,1:dim(HILICPOS)[2]]))$explvar$pvar * 100, names.arg = 1:nrow(HILICPOS))
...
## determination by cross-validation of the optimal number of latent variables 
## of the model 
...
lower than the cross-validated error obtained on the orignial data.
...
# model analysis, based on the scatter plot, the loading plot, the Variable 
...
plot(modelsoplsda$vip[[2]][order(modelsoplsda$vip[[2]][,2], decreasing = TRUE),1,drop=FALSE], pch = 16, ylab = "VIP", xlab="HILICPOS variables")
