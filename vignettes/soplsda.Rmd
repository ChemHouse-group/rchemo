...
Plasma were analyzed by several metabolomics platforms. In our example, the 
objective is to highlight metabolites discriminating males and females.
...
# Check dimension
BlockNames <- c("GCTOF", "HILICPOS", "metadata")
...
plotxy(X= pcaHILICPOS$T, group = sample_metadata$Gender, 
       ...
       zeroes = TRUE, circle = FALSE, ellipse = FALSE,
       labels = FALSE,
       legend = TRUE, main = "scores - PCA HILIC POS", ncol = 1,
       pch=16)
...
- nlvlist: if step is not "nlvtest". A list of same length as the number of 
...
variables of each block, and the corresponding error rates and sum of latent
variables.
...
NOTE: In our example, only the HILICPOS matrix is retained by the SO-PLS-DA 
method to discriminate the groups.
If both matrices had been retained, there would have been two loading plots and
two VIP curves, and the scores obtained from the two matrices would have been
concatenated.
...
plotxy(do.call("cbind",modelsoplsda$scores)[,1:2], group = sample_metadata[,"Gender"],
       ...
       zeroes = TRUE, circle = FALSE, ellipse = FALSE,
       labels = FALSE,
       legend = TRUE, main = "scores - SO PLS DA", ncol = 1,
       pch=16,
       xlab="lv1 HILICPOS",
       ylab="lv2 HILICPOS")

# loading plot
plotxy(modelsoplsda$loadings[[2]], pch = 16, xlab="lv1 HILICPOS", ylab="lv2 HILICPOS",
       main = "loadings - SO PLS DA")

# vip curve
plot(modelsoplsda$vip[[2]][order(modelsoplsda$vip[[2]][,2], decreasing = TRUE),1,drop=FALSE], pch = 16, ylab = "VIP", xlab="HILICPOS variables", main = "VIP curve SO PLS DA")
